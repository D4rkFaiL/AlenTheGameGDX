...
public class Ratao extends Enemy{
    ...
    public Ratao(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new Array<TextureRegion>();
        for (int i = 0; i < 4; i++) {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("RatinhoAndando"),i*31,0,31,11));
        }

        walkAnimation = new Animation(0.1f,frames);
        stateTime = 0;
        setBounds(getX(),getY(),31/Alen.PPM,11/Alen.PPM);
    ...
    public void update(float dt){
        ...
        if (setToDestroy && !destroyed){
            ...
            setRegion(new TextureRegion(screen.getAtlas().findRegion("RatinhoAndando"),31,0,31,11));
