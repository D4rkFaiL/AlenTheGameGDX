...
public class Morcego extends Enemy{
    ...
    public Morcego(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new Array<TextureRegion>();
        for (int i = 0; i < 6; i++) {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("BatmanVoando"),i*42,0,42,29));
        }

        walkAnimation = new Animation(0.1f,frames);
        stateTime = 0;
        setBounds(getX(),getY(),42/Alen.PPM,29/Alen.PPM);
    ...
    public void update(float dt){
        ...
        if (setToDestroy && !destroyed){
            ...
            setRegion(new TextureRegion(screen.getAtlas().findRegion("BatmanVoando"),42,0,42,29));
    ...
    protected void defineEnemy() {
        ...
        vertice[0] = new Vector2(-6,5).scl(1/Alen.PPM);
        vertice[1] = new Vector2(6,5).scl(1/Alen.PPM);
        vertice[2] = new Vector2(-6,-5).scl(1/Alen.PPM);
        vertice[3] = new Vector2(6,-5).scl(1f/Alen.PPM);
        shape.set(vertice);
